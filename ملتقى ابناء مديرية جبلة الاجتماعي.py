...
def init_db():
  ...
  # ضمان وجود الأعمدة الجديدة حتى وإن كان الجدول قد أُنشئ قديمًا
  new_cols = [
      ("delegate_name", "TEXT"),
      ("delegate_sub", "TEXT"),
      ("other_needs", "TEXT"),
  ...
  for col_name, col_type in new_cols:
    try:
      cursor.execute(
          f"ALTER TABLE refugees_full ADD COLUMN {col_name} {col_type}"
      )
...
with st.sidebar:
  st.markdown(
      "<h2 style='text-align: center; color: #075E54;'>🏢 ملتقى جبلة"
      " الاجتماعي</h2>",
      unsafe_allow_html=True,
  )
  st.write("---")
...
if choice == "📊 لوحة التحكم الإحصائية":
  st.markdown(
      "<h1>📊 لوحة التحكم الموحدة للملتقى</h1>", unsafe_allow_html=True
  )
  st.markdown(
      "<h3>الجمهورية اليمنية - ملتقى أبناء مديرية جبلة - اللجنة"
      " الاجتماعية</h3>",
      unsafe_allow_html=True,
  )
  st.write("---")
  ...
  with st.form("full_refugee_form", clear_on_submit=True):
    ...
    with r6:
      displacement_count = st.number_input(
          "عدد مرات النزوح:", min_value=1, step=1
      )
    ...
    with m1:
      m_under_1 = st.number_input("ذكور أقل من سنة:", min_value=0, step=1)
    with m2:
      m_1_5 = st.number_input("ذكور 1-5 سنوات:", min_value=0, step=1)
    with m3:
      m_6_17 = st.number_input("ذكور 6-17 سنة:", min_value=0, step=1)
    with m4:
      m_18_59 = st.number_input("ذكور 18-59 سنة:", min_value=0, step=1)
    with m5:
      m_60_plus = st.number_input("ذكور 60+ سنة:", min_value=0, step=1)

    st.markdown("**توزيع الأفراد الإناث:**")
    f1, f2, f3, f4, f5 = st.columns(5)
    with f1:
      f_under_1 = st.number_input("إناث أقل من سنة:", min_value=0, step=1)
    with f2:
      f_1_5 = st.number_input("إناث 1-5 سنوات:", min_value=0, step=1)
    with f3:
      f_6_17 = st.number_input("إناث 6-17 سنة:", min_value=0, step=1)
    with f4:
      f_18_59 = st.number_input("إناث 18-59 سنة:", min_value=0, step=1)
    with f5:
      f_60_plus = st.number_input("إناث 60+ سنة:", min_value=0, step=1)

    st.markdown("**الإجماليات والحالات الخاصة:**")
    tot1, tot2, tot3 = st.columns(3)
    with tot1:
      total_family = st.number_input(
          "إجمالي أفراد الأسرة:", min_value=1, step=1
      )
    with tot2:
      disabled_count = st.number_input("عدد المعاقين:", min_value=0, step=1)
    with tot3:
      sponsored_count = st.number_input("عدد المكفولين:", min_value=0, step=1)
    ...
    if submit_btn:
      if head_name:
        try:
          ...
          # الاستعلام الصحيح (تم تعديل علامات الاستفهام لعدد 62 دقيق)
          query = """
...
# --- 3. قسم تغيير كلمة المرور ---
  ...
  with st.form("change_password_form"):
    st.subheader(
        f"تغيير كلمة المرور للحساب: **{st.session_state['username']}**"
    )
...
# --- 4. عرض البيانات ---
  ...
  st.markdown(
      "<h1>🔍 قاعدة بيانات الاستمارات المسجلة</h1>", unsafe_allow_html=True
  )
  st.write("---")
  conn = sqlite3.connect(DB_NAME)
  df = pd.read_sql_query(
      "SELECT id as 'ت', doc_number as 'رقم الاستمارة', head_name as 'اسم رب"
      " الأسرة', phone as 'رقم الهاتف', orig_gov as 'المحافظة الأصلية',"
      " total_family as 'عدد الأفراد' FROM refugees_full",
      conn,
  )
  conn.close()
  if not df.empty:
...
# --- 5. قسم توزيع السلال الغذائية ---
elif choice == "📦 توزيع السلال الغذائية":
  st.markdown(
      "<h1>📦 قسم إدارة وتوزيع السلال الغذائية</h1>", unsafe_allow_html=True
  )
  st.write("---")
  ...
  with col1:
    ...
    with st.form("food_form", clear_on_submit=True):
      beneficiary_name = st.text_input("اسم المستفيد:")
      phone_food = st.text_input("رقم الهاتف:")
      basket_type = st.text_input("نوع السلة:")
      dist_date = st.text_input(
          "تاريخ التسليم:", value=datetime.now().strftime("%Y-%m-%d")
      )
      notes = st.text_area("ملاحظات:")
      save_food = st.form_submit_button("حفظ عملية التسليم")
      ...
      if save_food:
        if beneficiary_name:
          ...
          cursor = conn.cursor()
          cursor.execute(
              "INSERT INTO food_baskets (beneficiary_name, phone, basket_type,"
              " dist_date, notes) VALUES (?, ?, ?, ?, ?)",
              (beneficiary_name, phone_food, basket_type, dist_date, notes),
          )
          conn.commit()
          conn.close()
          st.success("تم تسجيل تسليم السلة بنجاح!")
        else:
          st.error("يرجى إدخال اسم المستفيد.")
