...
# --- 📝 تعبئة استمارة جديدة ---
  ...
  st.title("📝 تعبئة استمارة نزوح ومسح جديدة")
  with st.form("full_refugee_form_main", clear_on_submit=False):
    st.subheader("📌 بيانات الترويسة والوثيقة")
    ...
    blood_type = c10.selectbox(
        "فصيلة الدم:", ["غير محدد", "A+", "A-", "B+", "B-", "AB+", "AB-", "O+", "O-"]
    )
    ...
    st.subheader("📍 البيانات الجغرافية قبل وبعد النزوح")
    st.markdown("**1. المحافظة الأصلية:**")
    ...
    st.markdown("**2. مكان قبل النزوح:**")
    p1, p2, p3, p4 = st.columns(4)
    prev_gov = p1.text_input("المحافظة (قبل النزوح):")
    prev_dir = p2.text_input("المديرية (قبل النزوح):")
    prev_sub = p3.text_input("العزلة (قبل النزوح):")
    prev_village = p4.text_input("القرية / الحارة (قبل النزوح):")

    st.subheader("🤝 أقرب صلة قرابة")
    ...
    st.subheader("📋 حالة النزوح للأسر")
    s1, s2, s3 = st.columns(3)
    family_status = s1.selectbox("حالة الأسرة:", ["نازح", "مقيم", "عائد"])
    displacement_date = s2.text_input("تاريخ النزوح للأسرة:")
    displacement_count = s3.number_input(
        "عدد مرات النزوح:", min_value=1, value=1
    )

    st.subheader("👨‍👩‍👧‍👦 عدد أفراد الأسرة بالتفصيل")
    spouse_name = st.text_input("اسم الزوج / الزوجة رباعياً:")

    st.markdown("**التوزيع العمري للذكور:**")
    ...
    st.markdown("**التوزيع العمري للإناث:**")
    ...
    total_family = tot1.number_input(
    ...
    house_num = h_c1.text_input("رقم البيت:")
    house_type = h_c2.text_input("نوع البيت:")
    house_ownership = h_c3.selectbox("ملك / إيجار:", ["إيجار", "ملك", "مستضاف", "مأوى موقت"])
    landlord_name = h_c4.text_input("اسم صاحب البيت المؤجر إن وجد:")
    house_gov = h_c5.text_input("المحافظة (للسكن):")
    landlord_phone = st.text_input("رقم الجوال (للمؤجر/السكن):")

    st.subheader("🆘 أهم الاحتياجات والمنظمات")
    st.write("حدد مستوى الاحتياج للخدمات التالية:")
    n1, n2, n3, n4, n5, n6, n7 = st.columns(7)
    need_shelter = n1.selectbox("مأوى:", ["لا يوجد", "ضروري", "عاجل"])
    need_supplies = n2.selectbox("مواد إيواء:", ["لا يوجد", "ضروري", "عاجل"])
    need_water = n3.selectbox("خزان مياه:", ["لا يوجد", "ضروري", "عاجل"])
    need_food = n4.selectbox("غذائي:", ["لا يوجد", "ضروري", "عاجل"])
    need_medical = n5.selectbox("طبية:", ["لا يوجد", "ضروري", "عاجل"])
    need_school = n6.selectbox("حقيبة مدرسية:", ["لا يوجد", "ضروري", "عاجل"])
    need_bathrooms = n7.selectbox("الحمامات:", ["لا يوجد", "ضروري", "عاجل"])

    st.markdown("**بيانات التسجيل بالمنظمات:**")
    o1, o2 = st.columns(2)
    registered_wfp = o1.selectbox("هل مسجل في الغذاء العالمي؟", ["نعم", "لا"])
    current_org = o2.text_input("ماهي المنظمة المقدمة حاليا؟:")

    other_needs = st.text_area("الاحتياجات الأخرى:")

    st.subheader("✍️ بيانات المندوب والاعتماد")
    del1, del2 = st.columns(2)
    delegate_name = del1.text_input("الاسم / مندوب عزلة:")
    delegate_sub = del2.text_input("العزلة:")
...
# --- 📥 مركز تصدير التقارير (Excel) ---
elif menu_option == "📥 المركز الموحد لتصدير البيانات والتقارير إلى Excel":
